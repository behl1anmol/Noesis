...
1. Run `uv run pytest tests/eval/ -m golden -s` (the golden-set harness). It loads
   the real embedder and reranker; the last recorded run took 15 m 25 s on a
...
2. Read `tests/eval/report_latest.md` — verdicts first, then provenance, then
   ...
   The verdict block also states whether the run **re-baselined** — and
   distinguishes a re-baseline that was *requested* from one that actually
   wrote the file, since a failed layer refuses the write. A run that wrote the
   reference gated nothing, so never read its tables as a pass.
3. Report the three layers as the harness decided them, not as you judge them:
   ...
   - Any query with zero recall on every channel. Five channels missing the
     same query is a label to inspect before it is a retrieval regression.
   - The per-query block (ADR-71): queries whose Recall@10 crossed zero against
     the reference, and any change to the zero-recall set. Reported, never
     gated — quote it as an observation, never as a verdict.
