...
> /milestone M8 — Build the human monitoring surface and automatic freshness, the observability half of the MVP (Overview §4.9, §6.5). Server-rendered Jinja2 dashboard at `GET /`: indexed projects, per-project file/chunk counts, last-index time, run status/progress, failed files — read through core over SQLite state, no logic in `api/`. `watchdog` observer auto-reindexes **only** the changed file within seconds on a watched project, started in the app lifespan alongside the asyncio worker. Exit: humans see index health AND freshness is automatic. Constraints: localhost-only (bind 127.0.0.1, rule 2), no outbound HTTP in `core/`. New runtime deps `jinja2` + `watchdog` are pre-decided in Overview §4.9/§4.10 — confirm /adr coverage before adding (rule 3). Anti-goal: no SPA build tooling, no interactive mutation beyond the existing `reindex` action.
