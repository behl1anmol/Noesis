...
**Deliberately absent:** any HTTP client for embedding (httpx was in rev 1 solely for the removed remote path — gone with it); `astchunk` (test oracle only, installed from git in dev-deps, never a runtime dependency); Celery/Redis; any ORM; `fastembed` (Qdrant-native BM25 replaces the client-side encoder unless the Could-have tokenization work triggers).
...
Smaller than rev 1 — the expanded scope now requires exactly **one** new column and zero new tables:
...
|---|---|---|---|
| 9 | Reranker latency/memory unacceptable on developer laptops | Medium | Lazy load, `enabled=false` kill switch, latency in the harness, default decided by M4 data |
...
| 12 | Git fast-path silently wrong after history rewrite | Medium | Anchor-commit reachability check → full-walk fallback; hash confirms every candidate; per-condition tests |
...
1. **(informs M4)** Latency budget for a reranked search (p95). Without a number, "acceptable" is unfalsifiable; propose 500 ms p95 on CPU as the default budget and adjust from M4 data.
