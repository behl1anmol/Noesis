...
│   └── LESSONS-archive.md       # retired + promoted lessons — COMMITTED, never injected (ADR-61)
...
`.claude/commands/eval.md` runs the harness and reports Recall@10 / NDCG@10 / latency plus the three gate verdicts, read out of `tests/eval/report_latest.md` rather than the terminal (ADR-65 — the tables are printed and the documented invocation is `-q`). It deliberately *cannot* re-baseline: that needs an env-var prefix its `allowed-tools` cannot express. `.claude/commands/adr.md` appends to the `decisions` table *and* the doc's decision log in one step, taking `$ARGUMENTS` as the title.
...
| Durability across clones/machines | `LESSONS.md` **and** `LESSONS-archive.md` are **committed**; the DB is gitignored | Lessons are team knowledge; session memories are machine-local. `lessons import` rehydrates a fresh clone's DB from both files, ids and statuses intact (ADR-61) |
...
5. **Retire.** Lessons contradicted by a later ADR, made obsolete by refactoring, or unused past a milestone boundary are flipped to `retired`. They leave the injected set and are rendered to the committed `dev/LESSONS-archive.md` instead (ADR-61) — the DB alone was not audit storage, being gitignored, so a fresh clone used to lose every retired and promoted lesson along with its rationale.
...
| 47 | Index-run cancellation waits out the in-flight upsert (2026-07-17) | `execute_run` wraps its per-file `store.upsert_chunks` in `asyncio.ensure_future` + `await asyncio.shield(upsert)`; on `CancelledError` it awaits the shielded future (`gather(..., return_exceptions=True)`) before re-raising. Cancellation aborts the run at the same point and ADR-41 per-file containment is untouched (`CancelledError` is a `BaseException`, so `except Exception` never sees it) — the run just waits out the one write already in flight. Paired with `core.dashboard.delete_project` becoming async and awaiting the cancelled task: both halves are required, neither works alone | asyncio delivers `CancelledError` at an `await asyncio.to_thread(...)` **immediately, while the worker thread runs on** — verified empirically, not assumed: after `task.cancel()` + `await gather(task)` the thread had not yet written, and its write landed afterwards. So `delete_project`'s cancel-then-wipe wiped first and the abandoned upsert re-inserted points under a `project_id` that no longer existed. Nothing reclaims them: every prune path is scoped to a live project. Storage-only (ids are `uuid4`, so a re-registered folder gets a fresh id and can never read stale points) but unbounded across repeats. Waiting costs one file's upsert (files capped at `max_file_bytes`), not the whole run, so a DELETE stays responsive | Cancel-and-await the task alone (the plan's first cut — provably a no-op: the thread outlives it); cooperative cancel flag checked per file (more plumbing through jobs/indexer for the same bound); accept the leak (unbounded across repeats) |
| 48 | Startup orphan-point sweep, refused on an empty project table (2026-07-17) | `VectorStore.delete_orphan_points(live_project_ids)` deletes points whose `project_id` is not in the live set (Qdrant `must_not` + `MatchAny` = the complement) and returns the count; `build_runtime_context` calls it once at startup off the event loop, warning when it removes anything. Returns 0 without touching the collection when the live set is empty | The Qdrant-side counterpart of `state.fail_orphaned_runs`: the backstop for orphans no wipe could prevent (a process killed mid-run, a run whose upsert outlived the loop), rather than only the delete/upsert race #47 closes at source. Startup is the only safe moment — no run of ours is in flight, and a project row is committed before its first point is written, so a project another transport is mid-indexing can never look like an orphan. The empty-set refusal is the safety property: an empty project table is **indistinguishable from a state DB resolved to the wrong path** (exactly the cwd-relative `db_path` bug #44 fixed) and this operation is destructive, so there "no live projects" would read as "delete the whole collection". A skipped sweep costs a few dead points until a project exists; a wrong sweep costs the entire index | Sweep unconditionally (turns a benign misconfiguration into total index loss); sweep inside `delete_project` (still races the in-flight write — the thing #47 exists to fix); periodic background sweep (repeats the destructive window with no added coverage) |

| 49 | SQLite↔Qdrant drift self-heal at index time (2026-07-17) | `execute_run` compares `VectorStore.count_project_points(project_id)` (exact, filtered) against `state.expected_chunk_total` (`SUM(files.chunk_count)`) at run start. On mismatch: full runs (`paths is None`) scroll `per_file_point_counts`, add files whose live count != stored `chunk_count` to the changed set (unioned into git candidates, widening-only like the H1 dirty union), and prune within-project orphan paths (present in Qdrant, absent from both state and disk); scoped watcher runs log a warning only. Re-embed is idempotent via the deterministic uuid5 point ids, so it restores missing points without duplicates. `ensure_collection` now returns whether it created the collection; `runtime` warns on the wipe signature (created while `indexed_file_total > 0`). `index_status` exposes `expected_chunks`/`vector_count`/`drift` | The state DB and vector store diverge silently when Qdrant loses a collection externally: SQLite still reports files indexed, SHA-256 hashdiff sees no content change, incremental runs write nothing, and search returns empty **permanently** (observed on mediatorlite 2026-07-13 — the collection was recreated empty 2026-07-12, 322 chunks stayed missing across every later run). Content hash was wrongly trusted as proof the vectors exist. Surgical per-file repair chosen: cheap steady state (one filtered exact count per run; scroll only on real drift), heals partial loss and clears orphans a blunt full re-embed cannot. Scoped runs stay warn-only to preserve their candidate scope and never-advance-anchor rule (#37/#40). No new runtime deps — qdrant-client `count`/`scroll` already present | Full re-embed on any mismatch (wasteful on partial drift, cannot clear orphans); detect-only + surface in status (gaps persist until an operator forces a full reindex); trust `chunk_count` as-is (the bug — permanent silent search gaps); add a `file_path` payload index for a facet-based count (schema change, invalidates the M5 path-filter deferral) |

| 50 | Documentation stack: MkDocs Material on GitHub Pages via Actions (2026-07-19) | Docs site in `docs/` + `mkdocs.yml`, built by mkdocs-material with mkdocstrings-python (griffe **static** analysis, `paths: [src]` — the package is never imported, so no runtime deps at docs-build time) and mermaid via `pymdownx.superfences`; deployed by `.github/workflows/docs.yml` (official `configure-pages`/`upload-pages-artifact`/`deploy-pages`) on path-filtered push-to-main, `release: published`, and manual dispatch; `scripts/docs/gen_releases.py` (stdlib urllib, CI tooling — outside the `core/` no-HTTP boundary) renders `docs/releases.md` from the GitHub Releases API, with a committed byte-identical zero-release placeholder so offline `mkdocs build --strict` stays green; new dev-only `docs` dependency group (`mkdocs-material`, `mkdocstrings[python]`) | Material is the de-facto Python docs standard (search, dark/light palettes matching the brand SVGs, native mermaid) and stays inside the uv toolchain as a dev-only group — rule 3 (no new runtime deps) untouched; static mkdocstrings keeps CI free of ML wheels; official Pages actions avoid a `gh-pages` branch and Jekyll ambiguity; the release-published trigger keeps the Releases page current with zero manual steps | Sphinx (reST-heavy for a markdown-first repo); Docusaurus (foreign Node toolchain); mike versioned docs (premature at zero releases); branch-based Pages deploy (extra branch, Jekyll processing ambiguity) |

| 51 | Discovery errors block deletion, never trigger it (2026-07-25; extended in PR #24 review) | **Extension:** the genuine-absence branch applies only to a *subdirectory* — `os.walk` routes the walk **root's** own scandir failure through the same hook, and an unmounted or renamed root arrives as `FileNotFoundError`, which reported an empty tree with zero errors and purged the entire index while reporting `done`. The root is recorded as `<root>` instead. Independently, a scan that returns **nothing** while state tracks files is refused as an unreadable root regardless of errno, because a root that exists but is empty walks cleanly and raises nothing at all. Directory rels and the `<root>` sentinel are kept in `run_file_errors` but out of `failed_paths`, since the caller re-pends those as file rows and a scoped run matches its candidates exactly. **Exceptions — REMOVED by ADR-58 (2026-08-01, issue #26).** This row previously named **two** carve-outs to "reported, never silently skipped", both logged at DEBUG and neither added to `DiscoveryErrors`: `_IgnoreStack.push`'s unreadable `.gitignore`, and the `stat` seeding the `follow_symlinks` cycle guard. There are now **none** — both are recorded, in `DiscoveryErrors.unscreened` and `.unidentified` respectively, and the two calls that used to *raise* EACCES/ESTALE out of the entire walk (`gitignore.is_file()` and the per-child `is_symlink()`) are caught. **One claim this row carried was wrong and is retracted:** it said losing an ignore file "cannot cause over-deletion" and risks only under-ignoring. It can cause over-deletion. Git is last-match-wins, so a deeper `.gitignore` can negate a shallower exclusion; when the deeper spec fails to load, the outer exclusion stands unopposed, the file drops out of `discovered`, and every consumer downstream reads that absence as deletion. That is why `unscreened` suppresses deletion under its prefix rather than merely being logged. The bounding still holds: the secret and generated skip-lists apply independently of the ignore stack, so this is never a secret-exposure path. See ADR-58 for the full taxonomy and for why identity faults stay non-suppressing. Base decision follows. | `discover_files` takes an optional `DiscoveryErrors` collector and stops swallowing filesystem faults: `os.walk(onerror=…)` and the per-file `stat` both discriminate genuine absence (`FileNotFoundError`/`NotADirectoryError`) from a transient `OSError`, recording only the latter. `hashdiff.partition` takes the file-level errors as `discovery_errored` and gives each the existing H7 treatment — the path joins `errored` and, when already known to `stored`, its stored hash is carried forward as unchanged so it can never reach `deleted`. A **directory**-level error suppresses deletion execution for the entire run and is merged into `hash_errors`, which blocks the git anchor from advancing through the existing gate. **Amended by ADR-60 (issue #28):** the deletion half is untouched and still absolute, but the anchor half was too blunt — a directory error whose paths can be named now advances the anchor carrying those paths in the H1 dirty set instead of freezing it project-wide. Scoped (watcher) runs additionally persist their successfully-indexed paths via `state.add_dirty_paths` | Discovery omission and real deletion are indistinguishable downstream — both present as "path absent from `seen`". A transient fault on a 9p/network mount (the primary dev setup, ADR-45) therefore deleted every chunk under an unreadable subtree, and advancing the anchor made the loss permanent by convincing the next fast-path run nothing had changed. Skipping deletion on doubt is the fail-safe direction and is the same discrimination H7 already applies to hashing and §3.2 rule 1 applies to candidate sets (widen, never narrow). Erring this way costs stale rows until the next clean run; erring the other way costs unrecoverable index loss. The scoped-run dirty persistence closes the paired H1 gap: a scoped run that indexes a path must record it as dirty, or a later revert-while-blind strands stale content | Keep swallowing per-file `OSError` (the bug — silent mass deletion); fail the whole run on any discovery error (one unreadable file blocks all other progress, contradicts ADR-41); delete anyway but re-add on the next run (the next run's fast path skips the subtree, so "next run" may never come) |

| 52 | Telemetry writes go to a dedicated writer thread; the usage page becomes eventually consistent (2026-07-25) | `record_query` resolves the DB path from the caller's shared conn, enqueues the row on a bounded `queue.Queue` (maxsize 1000) and returns. One daemon writer thread owns a connection per DB path (`state.connect`, `busy_timeout=250`) and is the only thread touching them, so no lock is involved; a full queue or a busy DB drops the row. `telemetry.close()` drains and stops the writer with a 5s bounded join and is called from `close_runtime_context` off the loop; `telemetry.flush()` is the read-your-writes barrier — it retries its barrier under the caller's own timeout when the queue is full rather than dropping it, since a maxed-out queue means a stalled writer, which is the one moment the barrier is worth having (PR #24 round-6 review). Dropping *rows* on overflow stays the contract; dropping the *barrier* would report "flushed" with the whole backlog pending. In-memory DBs still write inline on the shared conn (test-only shape) | The first cut moved the INSERT off the event loop but left it **awaited in the request path**, so under the exact dual-transport contention it targeted the response still waited up to the 250ms `busy_timeout`, and a process-wide write lock serialized concurrent searches behind each other (PR #24 review) — making the module's "fire-and-forget" claim untrue. Enqueueing makes it true: the query path pays one queue put, and a single consumer removes the lock contention instead of relocating it. The bound stops a stalled writer from growing memory, and dropping on overflow matches the standing contract that telemetry may lose a row but never slow a query. The explicit close path also stops the writer's handles outliving `ctx.conn` — a second open handle to the state DB blocks temp-directory cleanup on Windows/WSL and survives a DB-file removal **Ownership superseded by ADR-59 (2026-08-02, issue #27):** the writer thread, the bounded queue, the drop-on-full contract and the eventual-consistency consequence all stand exactly as recorded here; what changed is that the writer is no longer a module global shared by the process but a `QueryTelemetry` instance owned by each `AppContext`, with a terminal `close()`. Read this row for the *why the write is asynchronous*, ADR-59 for *who owns the writer*. | Keep the awaited write (leaves a real per-request cost under the very contention the fix targets); `asyncio.create_task` per row (removes the latency but keeps the 250ms wait and lock contention, and loses in-flight rows at shutdown with no drain); unbounded queue (a stalled writer grows memory without limit) |
| 53 | Drift is reported only when the expected total holds still across the count (2026-07-25) | `jobs.index_status` reads `expected_chunk_total` on both sides of the Qdrant count round-trip and reports `drift` only when the two readings **agree with each other** and disagree with the count; a total that moved is treated as an in-flight run. The response carries the fresher reading. Documented in the docstring and the MCP tool reference | Making `index_status` async (so the state reads run on the loop) put a scheduling point between the two halves of the comparison. `execute_run` writes points via `upsert_chunks` **before** committing `chunk_count` via `upsert_file`, so mid-run the count sits *above* the expected total and BOTH readings differ from it. The first attempt at this guard asked only whether *either* reading happened to equal the count — which gave the in-flight case two chances to coincide and still reported drift on a healthy index (PR #24 review, reproduced: expected 400 then 450 against a count of 470 → `drift=True`). Requiring agreement is the test that actually separates a moving total from a stable disagreement. Accepted cost: while a run is committing, `drift` reads false even if the store genuinely lost data, and the first call after the run settles reports it — a false negative for seconds beats a false positive on every status poll during every index run, since `drift` is user-visible on the dashboard and in `get_index_status` | Compare each reading against the count independently (the bug — an in-flight run still reports drift); suppress drift whenever a run row is `running` (misses a run owned by the other transport that finished between the two reads, and adds a third state read); accept the false positives (trains operators to ignore the one signal ADR-49 exists to raise) |
| 54 | Discovery-error suppression is scoped to the unwalked subtree (2026-07-26; refined in the PR #24 round-6 review) | A directory-level discovery error suppresses deletion and orphan pruning only for stored paths under that directory's root-relative prefix (`indexer._unwalked_prefixes` + `_under`), not for the whole project. Whole-run suppression is retained exactly where no prefix describes what went unseen: the `<root>` sentinel (walk root failed, or the empty-scan guard fired), a failure recorded with a path outside the root (`relative_to` failed), and `follow_symlinks=True`. **Refinement:** the empty-scan guard no longer fires on top of a recorded directory error (ADR-55), so a walk that enumerated no files because the only subtree holding them was unreadable keeps prefix scoping instead of widening to the whole run — an attributable failure stays attributable regardless of how much of the tree it happened to hide. **Superseded in part by ADR-60 (issue #28):** anchor advance is no longer blocked by every discovery error — an *attributable* one now advances the anchor while carrying the stored paths it hid in the H1 dirty set. The whole-run suppression of DELETION described above is untouched; only the anchor half moved. Anchor advance is still blocked outright for the three unattributable shapes named here | ADR-51 suppressed deletions project-wide on any dir error, but `os.walk` loses only the failing directory's own subtree and carries on elsewhere — deletions outside it were provable all along. A **permanently** unreadable directory (root-owned dir, dead network mount) therefore froze deletion processing and orphan pruning for the whole project forever, re-pended every path it hid on every run, and blocked the anchor for good (that last consequence is what ADR-60 removes, by carrying the unverified paths forward rather than by holding the anchor), with no signal separating "transient, will self-heal" from "latched" (PR #24 review). Containment is tested on the path separator rather than `startswith`, so a sibling `pkg2/` whose walk succeeded is not swept in. `follow_symlinks` voids prefix reasoning outright: the same directory is reachable under rel paths that are not under its own | Keep project-wide suppression (the latching bug); expire the suppression after N consecutive failures (a timer that purges on doubt — the thing PIPE-1 exists to refuse); record every path hidden by the failed walk (impossible — the subtree is precisely what was never enumerated) |
| 55 | The empty-scan guard gets two escapes: entries-seen and an operator force (2026-07-26; refined in the PR #24 round-6 review) | `discover_files` reports `errors.entries_seen` — file entries the walk enumerated, counted **before** any filter. The empty-scan refusal fires only when it is 0 **and discovery recorded no directory error of its own**; a walk that saw files and filtered them all is a readable, populated root, so the emptiness is a scope decision and those files leave the index. **Refinement:** the guard covers emptiness that raises nothing, so where an error was raised it defers — the recorded error names the directory and carries the real errno, while a synthesized `<root>` row would overwrite it (`record_file_errors` is INSERT OR REPLACE on `(run_id, path)`), double-count one failure in `files_failed`, and, under `force`, log a purge the deletion block then correctly refuses to perform. The escape's log line additionally splits `entries_seen` into entries the filters removed and entries that could not be read: the counter runs before screening, and an unreadable file is carried forward by hashdiff rather than deleted, so crediting it to the filters overstates what the run will do. Plus `force` on `execute_run`/`launch_index_run`, exposed as `?force=` on `POST /projects/{id}/reindex` — and **only** there, not on the MCP `reindex` tool — which accepts a genuinely empty root. `force` relaxes nothing else — a directory that failed to scan still suppresses deletions under it. Both escapes log a warning naming which one applied | The ADR-51 empty-scan guard could never be satisfied, so a project that legitimately reaches zero indexable files could not converge: every run failed, deletions were never applied, the anchor never advanced, and the only escape was deleting and re-registering the project. Narrowing a project's index scope (ADR-42) triggers it with a perfectly readable root, not just an unmounted one (PR #24 review). `entries_seen` separates those two from inside the walk, which the returned list cannot. Nothing observable separates "every file deleted" from "root not mounted", so that call is the operator's — an explicit flag, not a heuristic. Kept off the MCP surface for the same reason it exists: the caller of an MCP tool is an agent, not an operator, and a tool description that explains how to set `force` is a decision procedure an LLM will follow — an agent reading a zero-file status could purge the whole project index in one call (PR #24 round-5 review). **Precision (round-8 review):** this is an affordance, not an access boundary — `verify_local_origin` passes when Origin/Referer are absent, so any local process, an agent with a shell included, can send `?force=true` itself. What the omission buys is that no tool description hands the model the procedure. REST is a human who can check whether the disk is mounted. **Correction (PR #24 round-7 review):** an earlier version of this row and of the tool comment said "curl or the dashboard". The dashboard's Reindex button does post to `POST /projects/{id}/reindex`, but it never sends `force`, so the escape is reachable only by a caller that sets the query parameter itself — curl or any other JSON-API client. A dashboard-only operator has no path to it, which matters because an unescaped guard fails the run every time, forever; `docs/reference/dashboard.md` therefore names the curl explicitly. Adding a forcing affordance to the dashboard was considered and declined for now: it is a whole-project purge, and a one-click button is a materially lower bar than a hand-written request, which is the bar the "operator asserts it" rationale above is buying | Auto-accept after N consecutive empty-but-readable runs (purge on doubt with a timer; N runs is minutes on a watched project); `force` alone with no `entries_seen` (makes an ordinary scope narrowing require a forced run). Residual risk accepted: an unmounted root whose underlying directory holds leftover files that all get filtered would read as a scope decision |
| 56 | **Unwalkable-directory ledger and quarantine** (2026-07-30, issue #25) | A new `unwalkable_dirs` table records every directory-level discovery failure per project — keyed on exactly what `DiscoveryErrors.dirs` reports (a root-relative prefix, the `<root>` sentinel, or an absolute path when unattributable) — with a consecutive-failure count, first/last seen, last error and `quarantined_at`. Each run increments the rows it reported and **deletes** the rows it proved walkable; "proved walkable" requires `unwalked is not None` (the walk was attributable at all) *and* the row not being `_under` this run's unwalked prefixes, so a `<root>` failure clears nothing. After `indexing.unwalkable_quarantine_runs` consecutive failures (default 5; `0` disables) the row is quarantined and the stored paths it hides are dropped from `IndexResult.failed_paths`, so `jobs._run` stops re-pending them and the `pending_changes` backlog drains. Quarantine **never** touches deletion — ADR-51/54 keep sole ownership of that, keyed on `unwalked` — so a quarantined subtree's chunks are kept and stay searchable. The staleness hole this would otherwise open is closed by **re-admission**: the run that finds a recorded prefix walkable again unions the discovered paths under it into its own candidate set (widening-only, §3.2 rule 1), so an edit made while the directory was unreadable is re-hashed by the very run that observes the recovery. Unattributable rows are excluded from that union — it would silently become a whole-index hash — and force a full walk through ADR-57 instead. The ledger is **read** before `partition` and **written** after the file loop, so a crash cannot delete a row whose re-admission never ran. Visible in `run_file_errors`, a WARNING log, `IndexResult.unverified_suppressed`, the `index_status` counts and a dashboard panel carrying the number of indexed files each directory hides. | PR #24's round-5 review kept the re-pend deliberately, because dropping it reopened a staleness hole: a file edited while its directory was unreadable has already lost its watcher event, so nothing would retry it. That reasoning assumed the re-pend was the only retry channel — it is not. `discover_files` walks the **whole tree on every run** (`paths` narrows only hashing), so every run is already a fresh probe of every directory, and re-admission on recovery is a strictly better channel: derived fresh from the ledger rather than depending on a pending row surviving. That is what makes bounding the retry safe, which the round-5 answer could not assume. The count is kept per **directory**, not per path as the issue sketched: 2 000 files under one broken directory share exactly one cause and their counters would move in lockstep, so per-directory is the right grain and needs no schema change to `pending_changes`. A table rather than a JSON column on `projects` because it needs per-row counters and timestamps, must be queryable for the dashboard, and must not be a read-modify-write blob on the event loop. Keeping quarantine strictly out of the deletion decision is what makes the change reviewable: nothing here can purge on doubt, the property PR #24 spent four rounds establishing. Measured pre/post on a 3-file subtree at threshold 3: backlog pinned at 3 forever → drains to 0 at run 3, with `files_deleted=0` and all 4 files still indexed throughout. | Per-path attempt counter on `pending_changes` (the issue's own sketch — wrong grain, 2 000 counters for one cause, and a schema change to a hot table); drop the re-pend outright (reopens the round-5 staleness hole); delete the subtree after N failures (purge-on-doubt, exactly what ADR-51 refuses); a `.noesisignore`-style exclusion (an ignore is a *purge* — right for a deliberate human exclusion, wrong for a machine-inferred "cannot read this"; tracked separately) |
| 57 | **Automatic promotion of a scoped run to a full walk** (2026-07-30, issue #25) | `jobs.launch_index_run` promotes a caller-scoped run to a full walk (`paths=None`) on any of three triggers: an **unattributable** `unwalkable_dirs` row exists (the whole index is unverified, so no scoped run can help); `indexing.promote_after_scoped_runs` scoped runs have started since the last **completed** full run (default 20); or the effective candidate set reaches `indexing.promote_candidate_fraction` of the indexed file count (default 0.25) — that set being `pending`, plus `dirty_paths` **only while the last full walk completed with no failures** (**amended in the PR #30 review, rounds 1-3**). Each is disabled independently with `0`. Never promoted under `force`. A new additive `index_runs.scoped` column, written at INSERT by `try_start_run` so a crashed run still counts, drives the counter; a **failed** full run does not reset it. `watcher.start` additionally launches one **full** catch-up run instead of a scoped one when a watched `auto_reindex` project has ledger rows. | Every automated trigger is scoped — `watcher._maybe_auto_reindex`, `watcher.start`'s catch-up, `dashboard.set_project_flags`' catch-up and `dashboard.reindex_pending` — and both drains of the H1 dirty set need a full run: `clear_dirty_paths` (renamed `set_dirty_paths` by ADR-60, which made it write the unverified remainder rather than always clearing) requires `candidates is None` and `set_last_indexed_commit` requires a commit, neither of which a scoped run can produce under ADR-40. So after a project's first full run the dirty set only grew, and each accumulated path was unioned back into the next scoped candidate set. Measured pre/post through the jobs layer: 1 → 7 across seven scoped runs and still climbing, versus a sawtooth draining to 0 on each promoted run. The policy lives in `launch_index_run` because that is the single launch path all four scoped callers already use, and the same place `paths` feeds **both** `execute_run` and `clear_pending_changes` — deciding it there keeps the run and its pending clear describing the same thing, where promoting inside `execute_run` would leave the caller clearing only the old scoped set after a walk that examined everything. Cost is modest and worth stating: discovery already stats and binary-sniffs every file on every run, so a promoted run is roughly 2× that run, not 100×. A failed full run must not reset the counter, or a project that fails every full walk would stop promoting entirely. `force` is excluded because it lets an empty scan purge the index, and applying that to a tree the caller deliberately scoped is not what the operator asserted. The `watcher.start` full catch-up exists because once quarantine drains the backlog a project can have **no** pending rows left, and every other automated trigger fires off a filesystem event — which a dead mount does not produce. **Amendment rationale (PR #30 review):** both drains are gated on `clean_run`, so after a full walk that reported any failure the dirty set is pinned — and counting a pinned set latched this trigger permanently at the first crossing, promoting every launch thereafter on a number promotion is structurally incapable of moving. That is the same "a guard that can fire forever has no exit" shape ADR-56 exists to remove, reached from the other side. Round 1 keyed the guard on `unwalkable_dirs` rows; round 2 showed that covers only `dir_errors`, one of `clean_run`'s three terms, so a persistently unreadable **file** pinned the set identically with no ledger row and latched it just the same. Keying on the outcome (`state.last_full_run_was_clean`) subsumes the ledger check rather than sitting beside it, and leaves `unwalkable_dirs` to trigger 1 where the question really is attributability. **Full** walk and not last run: a scoped run never hashes outside its candidate set, so an unhashable file leaves scoped runs reporting clean while the fault is live, which made promotion alternate. Measured at shipped defaults over 12 launches — healthy 1/12; unreadable directory 5/12 → 0/12 → 1/12; unreadable file 5/12 → 5/12 → 1/12 — all three converging on the healthy baseline, the one remaining promotion being the full walk that discovers the fault. Known gap, documented rather than coded: the re-enabling clean walk comes from triggers 1/2, so disabling `promote_after_scoped_runs` removes this trigger's own recovery path. **Correction (PR #31 review):** that round-2 wording — `clean_run` ⟺ `files_failed == 0` — was a *derivation*, and it drifted the moment ADR-58 added a fourth term. Screening-held deletions gate both drains but are deliberately excluded from `files_failed`, because nothing failed to index; a degraded full walk therefore reported `files_failed == 0` while draining nothing, and this trigger counted a pinned dirty set again — the exact latch round 2 closed, reopened through a term it could not see. `last_full_run_was_clean` now reads a PERSISTED `index_runs.clean`, written by the one place that computes the verdict, with the old derivation kept only as the fallback for rows predating the column. Two readers re-deriving one writer's conclusion had drifted twice; this makes the next added term safe by construction. | Reinterpret `candidate_count`/`fast_path_used` instead of adding a column (their meanings are already load-bearing for the dashboard and usage page — the round-7 thread flagged changing them as a deliberate user-visible change); promote inside `execute_run` (desynchronises the run from its pending clear); promote on every run while a project has ledger rows (turns every watcher cycle into a full hash); a background timer/scheduler (a whole new lifecycle for what an existing launch seam already covers) |
| 58 | **Screening faults are recorded, non-fatal, and block only what they put in doubt** (2026-08-01, issue #26) | `DiscoveryErrors` gains two lists for faults where the walk completed and enumerated every file, but a *screening input* failed — so the result may be the wrong SHAPE rather than incomplete. **`unscreened`** (`gitignore.is_file()` and `_IgnoreStack.push`'s `read_text`) suppresses deletion and orphan pruning under its own directory prefix, reusing ADR-54's `_error_prefixes`/`_under` machinery, and blocks the git anchor **only when it actually held a deletion back**. (ADR-60 keeps that test and changes the consequence: a held-back deletion is now carried in the H1 dirty set and the anchor advances, so the same file is re-examined next run without latching the project.) **`unidentified`** (the per-child `is_symlink` test and the `follow_symlinks` cycle-guard `stat`) suppresses nothing. Neither counts toward `files_failed`, neither marks a run `failed`; both are written to `run_file_errors` under synthetic `<screening>:` / `<identity>:` keys and counted in `IndexResult.discovery_degraded`. All three sites now catch `OSError` rather than raising, which also stops one unstattable path 500-ing `structural_search` and breaking the ADR-42 registration preview. Supersedes the two carve-outs ADR-51 named: nothing in discovery is logged-but-unrecorded any more. **The run's `clean_run` verdict is now PERSISTED** on a new additive `index_runs.clean` column and read back by `state.last_full_run_was_clean`, replacing its `files_failed == 0` derivation (NULL = predates the column, falls back to the old rule). | **Corrects a mechanism this project had shipped as fact.** Issue #26 and the ADR-51 row both asserted a lost `.gitignore` "cannot cause over-deletion" and risks only under-ignoring. Git is last-match-wins, so a deeper spec can NEGATE a shallower exclusion (`*.gen.py` at the root, `!important.gen.py` below): lose the deeper spec and the outer exclusion stands unopposed, the file leaves `discovered`, and `hashdiff.partition` reports it `deleted`. Verified both ways — with both specs the file is kept; with the child unreadable its chunks are purged (`test_lost_gitignore_negation_does_not_delete_the_file`, which fails on `bfa5104` with `files_deleted=1`). So "log it and keep going" leaves a purge-on-doubt path open, the exact class ADR-51 exists to close — hence deletion suppression. But the walk was COMPLETE, so this must never reach `errors.dirs`: that feeds the ADR-56 ledger and would quarantine a directory that walked perfectly well, which is ADR-54's over-correction reached from the other side. Identity faults are a separate list because they can only ADD paths (a subtree walked twice under two rel paths, a link descended into) and so are never evidence a file is gone — the round-7 finding, preserved intact. The anchor is gated on `screening_held` rather than on the fault, so a permanently unreadable `.gitignore` that excludes nothing indexed does not latch the project onto full walks forever (the "guard that can never be satisfied" shape ADR-55 was written to avoid). The synthetic keys are load-bearing: `is_file()` reaches `stat()`, so a directory missing execute permission fails BOTH the screening test and the per-file stat of the same `.gitignore`, and `record_file_errors` is INSERT OR REPLACE on `(run_id, path)` — the round-6 collision that already cost an operator a real errno once. | Log only and keep going (leaves the negation-loss deletion open, and leaves the ADR-51 wording wrong); record as a directory error in `errors.dirs` (correct on deletion, but quarantines a walkable directory, counts a non-failure in `files_failed`, and puts a walked directory in the dashboard's "unwalkable" panel); prune the subtree and fail the directory (safest on both axes, but the subtree stops being indexed at all and the anchor latches — an availability cost for what is otherwise a preference violation); per-file errors for everything under the directory (creates a file-level retry backlog that ADR-56's directory-keyed quarantine cannot drain — strictly worse than the option above); keep a cached copy of the last-good ignore rules (a stale ignore spec is its own correctness hazard, and needs schema plus an invalidation rule) |
| 59 | **The telemetry writer belongs to its `AppContext`, and `close()` is terminal** (2026-08-02, issue #27) | `core/telemetry.py`'s writer state moves out of module globals (`_worker`, `_queue`, `_worker_lock`) into a `QueryTelemetry` instance that `AppContext` owns as `field(default_factory=QueryTelemetry)` — so all 15 construction sites, 13 of them in tests, keep working unchanged. One queue per instance, built in `__init__`; `close()` sets an instance `_closed`, is terminal and idempotent, and `_submit` tests that flag under the same lock `close()` takes, so a `record_query` arriving after teardown drops its row instead of starting a fresh worker. `record_query` keeps its `conn` parameter, used on the caller's thread only — to resolve the DB path, or to write inline for the in-memory shape a second connection cannot reach. `_writer_loop` stays a module-level function taking only the queue. `close_runtime_context` closes `ctx.telemetry`. **All three of the module docstring's carve-outs are deleted rather than explained:** the one-live-`AppContext`-per-process invariant, the deliberately non-terminal `close()`, and the queue owned by the worker *generation*. | The writer's state was module-global while its lifecycle was `AppContext`-scoped: `close_runtime_context` closed per context, but what it closed was process-wide, and all three carve-outs traced to that one mismatch (PR #24 round-8 review). `LocalSTEmbedder` has the identical shutdown race one directory over and carries none of them, because its state is per-instance — so terminality is **free** here. The round-7 objection to a terminal flag, *"at the cost of a module-global that every test building a second context would have to reset"*, described a property of the global, not an argument against terminal state: a second context now simply constructs a second writer, open from birth, with nothing to reset. This replaces the only part of PR #24 that rested on an invariant rather than a mechanism. Measured pre/post at `5515e9f`: a `record_query` arriving after teardown left **1** file descriptor on the state DB, held by a resurrected writer that nothing would ever close — exactly the cost the old docstring named; after the change, **0**, because the row is refused. A refused row is a DEBUG log and a return, never a raise: telemetry's top contract (never fail *or delay* the search) outranks terminality, which is why `LocalSTEmbedder._submit` raises where this does not — a dropped embedding is a correctness loss, a dropped telemetry row is this module's documented behaviour. Terminality also makes the round-3 generation machinery unnecessary: exactly one sentinel is ever enqueued and only `_writer_loop` consumes one, so `close()`'s abandoned-writer sentinel re-arm is removed as unreachable. The bounded join and the "do NOT drain a timed-out join" rule stay — those are MCP-1 and the round-5 fix, not ownership caveats. Mutation-tested: reverting terminality, the per-context ownership, the no-drain rule, or the barrier-under-lock rule each fails exactly the test written for it (6 of the 12 new tests fail when contexts share one writer again). | Keep the global and add a module-level terminal flag (round 7's rejected option — the reset hook every test would need is precisely the cost instance state removes, and it leaves the other two carve-outs standing); a process-wide singleton keyed by DB path (same lifecycle mismatch, more bookkeeping, and still no answer to which context owns teardown); bind the conn at construction so `record_query` takes no connection (reads better, but adds a `bind()` step and a new failure mode where anything swapping `ctx.conn` writes to a dead path); keep module-level `record_query`/`flush`/`close` wrappers over a default instance for compatibility (preserves the exact global this removes); leave it as documented design debt (what #27 exists to end, and the trigger it named — a second live context — is what any in-process harness, multi-project daemon or embedded mode creates) |
| 60 | **The git anchor advances past a failure whose paths can be NAMED** (2026-08-03, issue #28) | `execute_run` stops gating its two position-recording branches on `clean_run` and gates them on `may_advance = not all_failed and unwalked is not None`, writing the paths the run could not verify — `unresolved` = `file_errors` ∪ `hash_errors` ∪ `unverified` ∪ `screening_held` — into the H1 dirty set as it goes. The git branch unions them with the working-tree-dirty set in `set_last_indexed_commit`'s single `UPDATE`; the non-git branch writes them through `state.set_dirty_paths` (renamed from `clear_dirty_paths`, which no longer only clears). `clean_run` itself, `index_runs.clean`, `files_failed`, `failed_paths` and the deletion policy are all untouched — this changes when a position is recorded, nothing about what a run reports or purges. One count-only INFO line reports the carry, deliberately not paired with `files_failed`, which excludes `screening_held` and printed "despite 0 failure(s)" on exactly the run the line exists to explain | The gate exists so the next fast path never skips a file whose state is unknown; it delivered that by refusing to move, so ONE permanently unreadable directory — or one unreadable **file**, which leaves no `unwalkable_dirs` row for anything to key on — froze `last_indexed_commit` for the whole project forever, and `compute_candidates` then diffed against an ever-older commit. Naming the doubtful paths and carrying them meets the same guarantee: `dirty_paths` already means "owed re-admission next run" and the H1 union feeds it back into the candidate set. The enumeration is exhaustive rather than hopeful — `diff.deleted` is `stored - seen` and the deletion block partitions all of it into `unverified`/`screening_held`/`provable`, the last leaving `stored` — and two further live-code facts make it cheap and safe: `hashdiff.partition` hashes any discovered file **unknown to `stored`** regardless of candidacy (so a file added inside a subtree while it was unreadable is picked up free on recovery, closing the "anchored past the commit that added it" hole), and a candidate that is not discovered is never hashed (so carrying an unwalkable subtree costs nothing while it stays unwalkable). The rule SUBSUMES the old one: when `clean_run` is true every term is empty and `unwalked` is `()`, so the carried set is empty and both branches behave byte-identically — a healthy project cannot observe this. Gating on what was actually put in doubt rather than on the fault is ADR-58's own rule, extended to the other terms: an unreadable directory hiding nothing indexed now carries nothing and stops latching the project. Measured, same script against `b80634c` and this branch, one permanently unreadable directory, 8 commits each touching a different file: candidate_count `1,2,3,4,5,6,7,8` with the anchor matching HEAD 0/8 → `1,4,4,4,4,4,4,4` with it matching 8/8, `files_deleted` 0 and all 11 files still indexed in both. Through `jobs.launch_index_run` over 12 scoped launches the H1 set goes from a climb that never drops to a sawtooth (`1,2,3,4,5,6,7,3,4,5,6,7`) draining to the carried floor on each promoted walk. **`state.last_full_run_was_clean` is deliberately NOT re-pointed at the new predicate**: a faulted walk now drains only to that floor, which the next run re-derives identically, so counting it toward `promote_candidate_fraction` would promote on a number promotion cannot move — the exact latch the PR #30 review closed over two rounds, one level down. **Known gap, documented rather than coded** (PR #36 review): a project with `follow_symlinks = true` keeps the original latch in full. `_error_prefixes` returns `None` on ANY directory error under that flag — prefix containment stops implying "unseen" once a subtree is reachable under several rel paths — so `unwalked is None`, `may_advance` is false, and issue #28 is simply unfixed there. Left alone because closing it is the same judgement as the `<root>` carve-out rejected in the next column, not a smaller one: `unverified` does enumerate the paths, but on a symlinked tree it names them under rel paths whose relationship to the fault the walk could not establish, so carrying them is a guess dressed as a receipt. It is a non-default per-project option (ADR-42), and the behaviour is stated in `internals/git-fast-path.md` | Fix only `dir_errors`, as the issue text literally proposed (covers one of `clean_run`'s four terms — a permanently unreadable FILE latches identically with no ledger row, the sibling instance PR #30 round 2 found for the promotion trigger); advance on unattributable failures too, which the same enumeration argument technically permits (under `<root>` the carried set is the ENTIRE index, so it would write every path in the project into a JSON column on every run to buy a next run that hashes everything anyway — and ADR-57 trigger 1 already forces a full walk there); persist the new verdict into `index_runs.clean` so the fraction trigger counts the dirty set again on faulted projects (re-opens the PR #30 latch: a broken subtree larger than the fraction would satisfy the threshold on its pinned floor every launch, forever); keep blocking and instead bound the staleness some other way — a new column, a TTL on the anchor (more state for a question `dirty_paths` already answers) |
| 61 | **The committed lesson files are the durable record; the DB is a local cache** (2026-08-03, PR #36 review) | Three changes to `.claude/scripts/devlog.py`, one design. (a) `render-lessons` REFUSES and exits non-zero when the committed files name a lesson this DB holds in no status, with `--force` as the override. (b) `lessons import` preserves the `id` written in the file and reads BOTH lesson files, restoring each row's recorded status; render orders by `occurrences DESC, id DESC`. (c) Retired and promoted lessons render to a second committed file, `dev/LESSONS-archive.md`, whose headings carry `status: retired|promoted` and which `session_start.py` does NOT inject. Amends ADR-27, which specified a single committed rendered artifact | Three failures, each demonstrated rather than argued. **(a)** `render-lessons` rebuilds a COMMITTED file from a GITIGNORED DB, so on any machine whose DB is behind the file it is a delete rather than a refresh — and it prints `rendered N lesson(s)` either way. It destroyed the tracked lesson set once already (lesson 11, 3 occurrences before this PR) and came within one command of doing so again during PR #36: the DB held 0 lessons against the file's 15. The guard keys on "the file names a row this DB has never seen" rather than on a COUNT comparison, because retiring or promoting legitimately lowers the active count below the file's — a count test would fire on precisely the two lifecycle operations §5.6 exists to encourage, while retired and promoted rows remain in the table, so an absent row is unambiguous. **(b)** The import matched `## [id]` and discarded it, re-inserting under AUTOINCREMENT, so an import→render cycle renumbered all fifteen headings — and the lesson bodies cite each other by number ("this is lesson 14's failure mode"), so a rehydrate silently repointed those references. Ordering on `created_at` compounded it: import cannot recover a timestamp the file never stored, so every rehydrated lesson tied on `now()` and the whole occurrence-1 group reshuffled, making a rehydrate read as a content change in review. Ordering on `id` — data the file itself carries — makes the round trip an identity. **(c)** §5.6 step 5 said a non-active lesson "stays in the DB for audit", which was only ever true on the machine that retired it; the DB is gitignored, so a fresh clone lost every lesson the project had decided against or graduated, and the reason. Verified by deleting the DB and rehydrating: lessons 10 (retired) and 11 (promoted) vanished outright. Measured end to end after the fix: DB deleted → render refuses, exit 1, files untouched → `lessons import` restores all 16 rows with ids, statuses and occurrence counts intact → a further render reproduces both files byte-for-byte. **Recorded late, and that is its own finding:** these shipped in `c8e884a`/`f0ffb8e` with no decision row, which rule 6 does not exempt tooling from — the PR #36 review caught it, and the miss is captured as its own lesson | Put the archive in `LESSONS.md` (`session_start.py` injects that file VERBATIM, so a retired lesson there keeps steering every future session — defeating the point of retiring — and the archive would grow without bound the injected surface the 15-lesson cap exists to bound); guard on a count comparison (false-positives on retire and promote, as above); leave non-active lessons in the DB only (ADR-27's status quo, shown false on a fresh clone); make `render-lessons` auto-import when it detects a stale DB (silently resurrects whatever the file happens to hold, which is the same write-if-missing provenance blindness lesson 8 records — refusing makes the operator look); a `git`-tracked `devlog.sqlite` (already rejected by ADR-27: binary diffs, merge conflicts, and it drags machine-local session memories into the repo) |
| 62 | **The qdrant client and server pins are one decision, enforced by a test** (2026-08-04, issue #37) | `docker-compose.yml` moves to `qdrant/qdrant:v1.18.3` and `pyproject.toml` gains an upper bound, `qdrant-client[fastembed]>=1.18.0,<1.19`. `tests/test_qdrant_pin_compat.py` holds them together in the DEFAULT suite: it reads the server pin off disk, takes the client version from the resolved environment, and asserts **two** predicates — the client's own `is_compatible`, and **same-major-same-minor**, which is this row's actual rule. Every other place the version is written down is pinned to compose by the same file: the `server-compat` CI job's image tag, `docs/getting-started/installation.md`, and `m6-agent-connection-guide.md`'s release URL. A `server`-marked tier plus that CI job re-check the pair against a live container, and the CI service carries compose's two hardening settings (`QDRANT__TELEMETRY_DISABLED`, 127.0.0.1-only ports), asserted here too | The two pins had drifted three minor versions apart (client 1.18.0, server 1.15.5) against the client's stated tolerance of one, so the app warned on every connection while the suite stayed green: `runtime.py` is the only remote-client construction in `src/`, and all 19 store-touching test files use `QdrantClient(":memory:")`, which skips the version handshake entirely. Which end moves was decided by Appendix C, which already records the intended pair as "qdrant-client 1.18.x / server >=1.15.2" — the client floor was never unexplained; the server pin is the end that fell behind. v1.18.3 (2026-07-17) is the newest release, clears the >=1.15.2 BM25 floor, and pairs exactly with the client. **Asserting `is_compatible` alone was not enough, and that is the substantive correction this row carries** (PR #41 review): the client's predicate returns True for a minor difference of 1, so a server bumped one minor ahead of the client passed all five original tests and both server-tier checks — measured, with compose and `ci.yml` at v1.19.0 against a 1.18.0 client. Issue #37 began at exactly one minor of skew before it grew to three, so the vendor's tolerance is blind to the state this bug starts in; a green built on it says "qdrant-client permits this", not "we decided this". The same-minor assertion is what makes the coupling mechanical; `is_compatible` stays so a future client *tightening* its rule is also caught. Pinning the two prose copies is the same argument one step out — the Colab guide previously asked the reader in a comment to keep its version equal to compose's, which is the mechanism the CI-tag test exists to replace, and a stale doc version walks someone into the cross-1.17 storage migration `installation.md` itself documents. Verified end to end: with both pins moved, `-m server` passes against a live v1.18.3 and the app's startup log carries zero compatibility warnings; and each guard has been observed red at its own boundary, not merely green — one-minor server bump, either doc left stale, the CI service's telemetry env removed, its ports returned to a bare mapping, and a variant tag (`-unprivileged`) present on one file only. **Stated limit, not closed here:** a functional smoke test proves the hybrid path *runs* against the new server; it does not prove the *ranking* is unchanged. The sparse channel's IDF is computed server-side (`Modifier.IDF`) and RRF fusion uses a constant the server fixes and `FusionQuery` does not expose, so a three-minor server jump can move retrieval quality without failing anything — and `tests/eval/test_golden.py` measures on `QdrantClient(":memory:")`, an implementation nobody queries in production. So the compose comment's "reviewed rather than silent" purpose is discharged for *correctness* and open for *quality*; closing it means running the golden harness against a real server, which belongs to #38 | `check_compatibility=False` (silences the one signal that the pairing is untested, and keeps the untested pairing); capping the client to 1.15.x (contradicts Appendix C's recorded snapshot and runs a three-minor-old client for no stated benefit); a comment asking both pins to move together (exactly what was already there, and it is how they drifted); asserting only `is_compatible` (the vendor's tolerance, not this row's rule — shown above to pass on a one-minor drift); leaving `-m server` local-only (the issue's structural half is that CI cannot see this) |
| 63 | **ADR-25's "nothing leaves the machine" binds the datastore, not just our code** (2026-08-04, found while verifying ADR-62) | `docker-compose.yml` sets `QDRANT__TELEMETRY_DISABLED=true`, asserted by `tests/test_qdrant_pin_compat.py` | Observed, not inferred: the shipped image carries `telemetry_disabled: false` in `/qdrant/config/config.yaml`, and the container logs `Telemetry reporting enabled, id: <uuid>` then posts to `https://telemetry.qdrant.io/` on start. Binding 6333/6334 to 127.0.0.1 governs traffic coming IN and does nothing about traffic going OUT of the container, so rule 2's port discipline never covered this. ADR-25 and the README state that no code, query or metadata leaves the machine; enforcing that only against `core/` (where the CI grep looks) left the one container this project ships reporting collection and configuration statistics outbound by default. Both states verified against v1.18.3: default logs `Telemetry reporting enabled`, with the variable set it logs `Telemetry reporting disabled` | Leave it on (contradicts the project's headline guarantee, and users would have no way to know); document it in prose instead of setting it (the default is the thing that ships, and a prose warning is not a default); block egress at the container level with `network_mode`/firewall rules (heavier, platform-specific, and breaks nothing else the container needs to do because it needs no egress at all) |
| 64 | **Golden retrieval labels address content, not line numbers** (2026-08-08, issue #38) | Each `relevant` item is a `path` plus an `anchor` — a substring occurring exactly once in that file — optionally widened by an `anchor_end`. `load_golden(path, root)` resolves them against the tree being measured and raises on zero, multiple or reversed matches; positional `lines:` is rejected outright. `tests/eval/test_golden_labels.py` re-resolves every anchor in the DEFAULT suite, checks each labeled file is one discovery actually indexes, and requires a `symbol` query's identifier to appear inside its own span. `tests/eval/migrate_labels.py` records the mechanical re-derivation of all 46 labels from the labeling commit `1d2bf06` | Measured, not suspected: mapping every labeled span from `1d2bf06` onto HEAD with `difflib` opcodes, **22 of 46 labels no longer overlapped their own content** (nl 8/17, symbol 5/14, structural 9/15). `matches` requires span overlap, so each rotted label silently forced its query to 0 — and three were not drift at all: `AppContext` moved from `app.py` to `runtime.py` in `34f2408`, and `nl-10`/`symbol-12` pointed past the end of a 58-line `config.py` on the day they were written, so two of forty queries contributed a guaranteed 0 **inside the stored M2 baseline itself**. The asymmetry is the argument for where the check lives: `structural_patterns`, in the same file under the same header, is pinned by an unmarked default-suite test and was correctly updated by three separate commits, while the `queries` beside it had no check and rotted. Maintenance follows enforcement, not intent — so the integrity check is unmarked, not behind `-m golden`, which runs neither in CI nor unattended. golden.yaml's own header already stated the rule its `queries` section violated: "Counts, not line numbers, are pinned: ordinary edits move lines". Width follows the question because `dedupe_by_path` keeps only the best-ranked chunk per file: an identifier lookup's answer is its definition site, but a region query labeled to a single line would score 0 whenever the right region was retrieved and merely ranked behind another chunk of the same file. The migration is committed rather than run-and-discarded because the anchors must be re-derivable, and it earned that: the integrity test caught two labels (`symbol-07`, `symbol-11`) the migration had anchored on body lines that did not contain the identifier being searched for | Rewrite the 46 ranges to today's positions (fixes the instance, not the class — nothing stops the next thirty commits re-rotting it, which is precisely what happened); drop `lines` and score on file match alone (any chunk of the 1244-line `indexer.py` would count, and `structural-03`'s two same-file items would collapse under dedupe to a permanent 0.5 cap); resolve a `symbol:` name through `ast` (Python-only, and measured — it maps four labels onto `execute_run[228,1244]`, ~80% of the file) |
| 65 | **The golden run gates by assertion, in three layers, and never writes a baseline implicitly** (2026-08-08, issue #38) | Layer 1: same-run relational asserts — hybrid beats dense on Recall@10 overall and does not lose it on the symbol subset (M3's exit criterion), and hybrid+rerank does not lose NDCG@10 to same-run hybrid (ADR-35's shipped claim). Layer 2: regression against one living `tests/eval/baselines/reference.json`, gated only when its provenance block says it is comparable — same embedding and reranker model, same store kind and version, same digest of the parsed questions, corpus within a 20% chunk band; otherwise the run FAILS with the tables marked NOT A GATE and the exact re-baseline command. A drop must exceed both a relative band (10% overall, 20% per category) and `1/n_queries` absolutely. Layer 3: `NOESIS_EVAL_REBASELINE=1` is the only write path, it is refused on a tracked-dirty tree and checked before the two-hour run, and layer 1 still asserts while re-baselining. Every run writes `report_latest.md` — verdicts first, then provenance, then tables, plus every query scoring 0 on every channel | The old gate could not fail on quality: its assertions were categories-present and values-in-`[0,1]`, so the run in issue #38 whose every metric had roughly halved reported `1 passed, 18 deselected`. The stated reason in that issue is wrong and is corrected here — this repo has had CI on every PR since before PR #36; the actual mechanism is `pyproject.toml`'s `addopts = "-m 'not integration and not golden and not server'"`, so the `tests` job deselects the golden tier before collection. That reason is the stronger one: "there is no CI" would be fixed by adding CI, whereas "the gate is deselected from the only suite CI runs" stays true however much CI you add. Layer 1 is corpus-independent by construction, which is why it always asserts — but it is provably **not sufficient**, and that is pinned as a test: in the degraded run dense 0.4125 < hybrid 0.5000 < rerank 0.5875, so every relation held while everything halved. Layer 2 is what catches uniform degradation, and its provenance guard is the fix for Finding B — recall against a fixed label set falls mechanically as the corpus grows (94 to 184 files, of which 63 are markdown, an indexed language, against a label set that is 100% `.py`), so an older reference is not a weaker reference but a different experiment. The absolute floor beside the relative band is arithmetic, not taste: one query moves the mean by `1/n`, which is a large *relative* figure at a low score and a small one at a high score, so a relative band alone silently changes its meaning with the score level. Failing rather than skipping on an incomparable reference is the forcing function the old design lacked; the cost of a false red is one human read of a report from a run they chose to start. The env var rather than a pytest flag buys a property worth having: `.claude/commands/eval.md` may only run `Bash(uv run pytest *:*)`, which cannot carry an env prefix, so `/eval` is structurally incapable of re-baselining. The keystone test replays issue #38's own measured numbers and asserts the gate rejects them (lesson 2: never assert before seeing the losing outcome) | Keep the human-read table (it is what failed — the documented invocation is `-q`, which suppresses it, and no CI job runs the tier); gate on absolute floors (a magic number where a measured reference exists); make the manifest hash a comparability predicate (it changes on nearly every commit, so the gate would never fire — Finding A rebuilt in a new costume, hence manifest-as-evidence); skip rather than fail on an incomparable reference (a silent skip is the exact shape of the bug being fixed); refresh `m2_dense.json` in place (it is the frozen evidence behind ADR-35 and is quoted in the docs; it also contains two labels that never matched, so it cannot be a like-for-like reference even with a fresh corpus) |
| 66 | **The store an evaluation ran against is part of its provenance** (2026-08-08, issue #38, closing ADR-62's open half) | `NOESIS_EVAL_QDRANT_URL` switches the golden `corpus` fixture from `QdrantClient(":memory:")` to a real server on a uniquely-named throwaway collection dropped in a `finally`, reusing `tests/test_qdrant_server_smoke.py`'s skip-if-absent REST probe. The store kind and server version are recorded in `reference.json` and are a hard comparability term | ADR-62 closed the qdrant pin question for *correctness* and said so explicitly, naming the residual as belonging to #38: "a functional smoke test proves the hybrid path runs against the new server; it does not prove the ranking is unchanged". The mechanism is specific — sparse IDF is computed server-side via `Modifier.IDF` and RRF fusion uses a constant the server fixes and `FusionQuery` does not expose — so a three-minor server jump can move retrieval quality while every existing test stays green, because all 19 store-touching test files use the embedded client, a Python reimplementation of the query semantics that never speaks the wire protocol. Making the store a comparability term is the load-bearing half: it is what stops an embedded-measured reference from silently gating a server-measured run, which would have been a new way to compare two different experiments | A separate `server`-marked golden tier (duplicates the whole corpus fixture; the env switch reuses one fixture for both stores); `check_compatibility=False`-style silence (rejected for the same reason ADR-62 rejected it — the warning is the signal); leaving the switch out and filing a follow-up (the fixture is already being rewritten here, and touching it twice across two PRs is worse than once) |
