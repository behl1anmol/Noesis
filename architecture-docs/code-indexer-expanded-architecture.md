...
| 62 | **The qdrant client and server pins are one decision, enforced by a test** (2026-08-04, issue #37) | `docker-compose.yml` moves to `qdrant/qdrant:v1.18.3` and `pyproject.toml` gains an upper bound, `qdrant-client[fastembed]>=1.18.0,<1.19`. The pair is asserted by `tests/test_qdrant_pin_compat.py` in the DEFAULT suite — it reads both pins off disk and applies the client's own `is_compatible` predicate — and re-checked against a live server by a new `server`-marked tier plus a `server-compat` CI job running the pinned image as a service container. The CI job's image tag is pinned to the compose tag by the same test, so the version cannot drift between the two files | The two pins had drifted three minor versions apart (client 1.18.0, server 1.15.5) against the client's stated tolerance of one, so the app warned on every connection while the suite stayed green: `runtime.py` is the only remote-client construction in `src/`, and all 19 store-touching test files use `QdrantClient(":memory:")`, which skips the version handshake entirely. Which end moves was decided by Appendix C, which already records the intended pair as "qdrant-client 1.18.x / server >=1.15.2" — the client floor was never unexplained; the server pin is the end that fell behind. v1.18.3 (2026-07-17) is the newest release, clears the >=1.15.2 BM25 floor, and pairs exactly with the client. The upper bound is what makes the coupling mechanical rather than remembered, and the compose comment's own stated purpose — that a server upgrade be reviewed rather than silent — is discharged by this row. Verified end to end: with both pins moved, `-m server` passes against a live v1.18.3 and the app's startup log carries zero compatibility warnings; with the server tag reverted to v1.15.5 the offline test fails naming both versions and both files | `check_compatibility=False` (silences the one signal that the pairing is untested, and keeps the untested pairing); capping the client to 1.15.x (contradicts Appendix C's recorded snapshot and runs a three-minor-old client for no stated benefit); a comment asking both pins to move together (exactly what was already there, and it is how they drifted); leaving `-m server` local-only (the issue's structural half is that CI cannot see this) |
