...
* `flush()` read the live generation under the lock, released it, and only
...
def _stop_the_writer():
    """Never leave a generation of this module's globals behind for the next
    test — these tests deliberately abandon a worker."""
    yield
...
async def test_flush_is_not_stranded_by_a_racing_close(tmp_path) -> None:
    """The window is between reading the live generation and putting the
    barrier. Constructing the barrier *before* taking the lock is what closes
    it: a `close()` that lands first is then seen by the lock-protected read,
    and `flush` returns instead of waiting out its timeout."""
    db_path = _db(tmp_path)
    telemetry._ensure_worker().put_nowait((db_path, dict(_FIELDS)))
    assert telemetry._worker is not None
    ...
    def racing_init(self) -> None:
        ...
        if not raced.is_set():
            raced.set()
            # Exactly the interleaving the old code allowed: the generation is
