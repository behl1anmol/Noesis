...
| 16 | Shared-server topology makes one process a single point of failure for every agent | Medium | Introduced deliberately by ADR-86 — each agent used to be independent. Accepted because a standalone agent costs ~1.14 GB of unshared model memory (ten agents ≈ 13.4 GB against ≈ 3.1 GB shared). The shim probes before proxying and re-elects on a dead endpoint, so a crash costs one cold start rather than a wedged fleet; `--no-spawn` suits operators who supervise the server themselves; and standalone remains the default |
| 17 | A future change reintroduces the shared-client race, which is silent and latching | Medium | The defect class that survived six review rounds, because the damage is wrong *results* rather than exceptions. Three layers: the design makes a raw client unreachable from search code; CI greps forbid constructing a client or building a `models.Document` outside their owners (ADR-83); and the regression tests assert correct results rather than the absence of exceptions — the missing oracle was the actual root cause |
