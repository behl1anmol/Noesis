...
| First *reranked* search very slow (minutes), with `[reranker] enabled = true` | Same cause one model over: the cross-encoder's ~2.3 GB of weights weren't fetched before startup. `uv run python -m noesis.prefetch` fetches them (either `--skip-reranker` or `--skip-model` leaves them out), and `reranker_assets`/`reranker_ready` on `/healthz` or `get_index_status` say which half is missing — cached weights, or the load itself. The reranker is warmed in the background too ([ADR-87](../project/decisions.md)), but deliberately *after* the embedder's warm-up finishes, so it is ready later on a cold start: the embedder gates every search, while the reranker only rescores hits a search has already produced |
