...
| `dirty_paths` | TEXT NULL | JSON list — everything the next run must re-admit as a candidate. Three populations: working-tree-dirty paths at the last anchor advance; the paths a scoped watcher run indexed, unioned in directly because such a run never advances the anchor and so never reaches the write that normally accompanies it; and — since [ADR-60](../project/decisions.md) — the paths a run could not verify (failed to index or hash, hidden under an unwalkable subtree, or held back from deletion by a screening fault). That third population is what lets the anchor advance past a nameable failure instead of freezing: the guarantee "no file whose state is unknown is skipped" is met by recording those paths here. The scoped-run union runs in a `BEGIN IMMEDIATE` transaction so two writers cannot lose each other's paths. Replaced wholesale by an anchor advance, or — for a project with no git anchor — by any full walk, which re-hashes everything it can reach and so leaves only the remainder owing |
...
| `clean` | INTEGER NULL | The run's own `clean_run` verdict — `1` when no file, hash, directory or screening-held term was set, `0` otherwise ([ADR-58](../project/decisions.md)). It gated both drains until [ADR-60](../project/decisions.md), which moved them onto a weaker predicate ("can the failure be named?"); this column kept its meaning and its one remaining reader, `last_full_run_was_clean`, deliberately — that asks whether a promotion could drain the dirty set *to empty*, and a faulted walk now drains only to the floor of what it could not verify. Persisted rather than re-derived: `last_full_run_was_clean` used to reconstruct it as `files_failed == 0`, which held only while every term was a file-level failure. That drifted twice — directory errors, then screening-held deletions, which are deliberately *not* counted in `files_failed` because nothing failed to index — and each time a reader claimed a run was clean when it had drained nothing. NULL means the row predates the column and falls back to the old derivation, which is correct for rows written before any term it missed existed |
