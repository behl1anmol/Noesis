...
Every close in that sequence runs under `await asyncio.to_thread(...)`, which preserves the ordering while keeping the joins off the event loop. Each model worker's `close()` joins its thread with a 5 s bound (a stuck load is abandoned, never waited on forever), and the telemetry writer has its own bounded join — invoked synchronously from an async teardown, those would stall the loop for up to 5 s apiece, and in the combined lifespan that stalls the MCP session manager's shutdown along with it. The telemetry writer must be stopped here rather than left to process exit: it holds its own handle to the state DB, which would otherwise outlive `conn`, block temp-directory cleanup on Windows and WSL, and survive a DB-file removal ([ADR-52](../project/decisions.md)). Stopping it also empties its queue — rows ahead of the shutdown sentinel are written, and anything that raced in behind it is discarded rather than left in a process-global queue where the *next* runtime's writer would pick it up carrying a dead DB path. With one exception: if the join times out, the queue is left alone. The writer still owns it and has not reached the sentinel, so draining would swallow the sentinel and leave that thread blocked on `get()` forever with its connections open — the leak this teardown exists to prevent, arriving through it. The writer is per *process*, not per `AppContext`, so this teardown assumes one live context per process — true by construction, since `app.py` and `mcp/__main__.py` each build exactly one and are separate processes. Two live contexts in one process would cost telemetry rows at the first teardown (they are dropped, and `flush` would then honestly report nothing pending); nothing else is affected.

That same invariant is what makes the writer's *resurrection* safe. `telemetry.close()` retires the worker generation but sets no terminal flag, so a `record_query` landing afterwards starts a new worker with a new handle — correct for a query that was already in flight when teardown began, and a handle leak for one arriving after teardown decided to close. The two are indistinguishable from inside the module. The window is the gap between stopping the writer and closing SQLite, and nothing reaches it: uvicorn drains in-flight requests before lifespan shutdown, `combine_lifespans` unwinds the MCP app first, and with one live context there is no other caller holding a `conn` to record against. A terminal flag would close the window at the price of a module-global that every test building a second context would have to reset ([PR #24](https://github.com/behl1anmol/Noesis/pull/24) round-7 review).
