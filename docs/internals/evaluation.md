...
Labels used to store `lines: [start, end]`. Because a result must overlap that range to count, a label whose code moved silently forced its query to zero — and by 2026-08, **22 of 46 labels had drifted off their own ranges**. Nothing detected it, while `structural_patterns` in the very same file stayed correct across three commits because a default-suite test pinned it. Ranges were refreshed exactly once, the day they were written — never again across the month of drift that followed (a "range past EOF the day it was written" count appeared in an earlier draft; the PR #42 round-5 review measured it against the wrong commit's file content and retracted it — see [ADR-64](../project/decisions.md)).
...
**The scorer is a comparability term ([ADR-70](../project/decisions.md)).** `provenance.scoring.version` says which scoring rules produced a stored number, and layer 2 refuses to compare across a change to it. Without that term the check covered models, store, labels and corpus size and said nothing about the thing computing the numbers — and ADR-67 and ADR-68 both changed what a retrieved chunk credits, each time relying on a human to remember to re-baseline. A reference recorded before the field existed reads as `None` and refuses in both directions, rather than passing because both sides are silent. Bump `SCORER_VERSION` whenever a change to `score_query`, `group_by_path` or `matches` can move a stored number. **Policy (PR #42 round-5 review, finding 7):** this bump turns the default suite red on every pull request until the ~15 GPU-minute re-baseline lands — the same standing incentive ADR-69 explicitly refused to build for label edits. Kept anyway, deliberately: a label edit is frequent and cheap and must not be discouraged, while a scorer change is rare and inherently invalidates the measurement, so silently comparing across it is the exact mistake this field exists to prevent. The rule that follows: **a scorer change and its re-baseline land in the same commit.**
...
!!! note "Frozen historical record"
    These numbers were measured on 2026-07-04, against the pre-[ADR-64](../project/decisions.md) positional label set, under which nl-10 and symbol-12 both scored 0 (a "~94 files" corpus figure and a claim that those two labels "could never match anything" appeared in an earlier draft; the PR #42 round-5 review measured neither against a committed tree and both were dropped rather than guessed). They are kept because they are the evidence behind ADR-35, and the *relations* they establish are still asserted by the gate's layer 1 on every run. They are **not** the live comparison; that is `baselines/reference.json`.
