...
- A **directory-level** failure means part of the tree went unwalked, and the paths it hid cannot be enumerated. The run therefore skips deletions *and* orphan pruning **for the paths under that directory**, and the paths it hid are carried in `dirty_paths` alongside the advancing git anchor, so the next run re-examines every one of them ([ADR-60](../project/decisions.md)). Before that they blocked the anchor outright, which met the same goal by freezing the project's fast path for as long as the directory stayed unreadable. Paths elsewhere in the tree were walked normally and stay provable ([ADR-54](../project/decisions.md)) — otherwise one directory that is permanently unreadable (root-owned, or a dead network mount) would freeze deletion for the whole project forever. Suppression widens back to the whole run whenever the failure cannot be attributed to a subtree: the walk root itself, a path outside the root, or `follow_symlinks`.
...
The asymmetry is deliberate. Skipping a deletion costs stale rows until the next clean run; performing one on bad evidence costs searchable content until a full reindex — and, once the anchor advances past it, silently. Note which half of that sentence carries the risk: it is the wrongful *deletion*, not the anchor. A path the run could not verify advances with the anchor and stays a candidate, so nothing is lost by moving on from it.
...
- **`last_indexed_commit`** is advanced after a full, successful run with a resolvable HEAD whose failures can be *named* — never after a run that finished `failed`, and never past a failure no path set describes (an unreadable walk root, a path outside the root, `follow_symlinks`). Whatever such a run could not verify is written into `dirty_paths` by the same statement that moves the anchor ([ADR-60](../project/decisions.md)).
