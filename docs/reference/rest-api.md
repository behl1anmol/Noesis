...
The reranker pair reads `"disabled"` when `reranker.enabled = false` (the shipped default). A value rather than absent keys, so a client can tell "reranking is off" from "this server doesn't report it", and no operator is sent to fetch 2.3 GB for a feature nobody turned on. All four read `"unknown"` when the app has no runtime context wired (a bare app without the lifespan) — a healthcheck reports what it cannot tell rather than raising.
