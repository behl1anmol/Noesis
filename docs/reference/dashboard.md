...
- **Model assets panel** — a "Download models" button that fetches tree-sitter grammars, the BM25 tokenizer assets, and the configured embedding/reranker model weights, with a live per-step progress bar (grammars → BM25 → model → reranker). The same one-time download `uv run python -m noesis.prefetch` does from a terminal, triggered here instead for an operator connecting to a shared/remote daemon over HTTP (issue #51). Progress is polled, not streamed — see [ADR row 95](../project/decisions.md) for why polling was chosen over Server-Sent Events.
