...
| `enabled` | bool | `false` | Kill switch **and** per-request default: `false` never loads the model and requests cannot opt in; `true` makes `rerank` default on with per-request opt-out ([ADR-34](../project/decisions.md)). Default-off is the measured M4 gate decision ([ADR-35](../project/decisions.md)). With `true`, `/healthz` and `get_index_status` also start reporting `reranker_assets`/`reranker_ready`, so weights that were never prefetched read `"missing"` instead of surfacing as a multi-minute stall inside the first reranked search ([ADR-87](../project/decisions.md)); with `false` both read `"disabled"`, and no HF-cache lookup happens at all. |
| `preload` | bool | `false` | `true` loads the ~568M-parameter (~2.3 GB) model inline during startup — the service does not answer until it is resident. `false` no longer means "on first reranked request": with reranking enabled, a background warm-up task loads it after startup instead ([ADR-87](../project/decisions.md)), queued *behind* the embedder's warm-up, because the embedder gates every search while the reranker only rescores hits a search already produced. So `false` costs a reranked search the load only when it arrives before that task finishes; `true` moves the wait to startup, where an operator is watching for it. |
